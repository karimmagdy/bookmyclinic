    ...
    <main className="max-w-md mx-auto px-4 py-10">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4 shadow-sm">
        ...
        <div>
          ...
          <p className="text-lg font-semibold">{formatDate((slot as { slot_date: string; slot_time: string }).slot_date)}</p>
        </div>
        <div>
          <p className="text-xs text-gray-400 uppercase tracking-wide">Time</p>
          <p className="text-lg font-semibold">{formatTime((slot as { slot_date: string; slot_time: string }).slot_time)}</p>
        </div>
